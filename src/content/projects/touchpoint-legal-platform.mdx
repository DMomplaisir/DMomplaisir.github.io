...
role: "Technical Lead"
impact: "Year-long 0-to-1 build, shipped AI email-to-task pipeline, created Outside Counsel Engagement Studio, YC interview"
---
...
The pitch was simple: a modern platform that actually understands how legal work happens, with AI that handles the administrative burden so legal professionals can focus on legal work. I came on as Technical Lead, taking on the challenge of building a startup while continuing to deliver at The Atlantic - a deliberate bet on exploring entrepreneurship. What started as helping a friend evolved into a year-long journey through product discovery, technical architecture, and the realities of early-stage startup dynamics.
...
1. **Semantic graph filtering** - Before any LLM call, we'd check: is this email even worth processing? Using pgVector embeddings, we built a graph of relationship connections and email patterns. Routine status updates, FYI forwards, calendar confirmations - the system learned to recognize and skip these without touching the LLM.

2. **Relationship pre-processing** - When we did need the LLM, we'd pre-compute the relationship context: who sent this, what matters are they on, what's their typical communication pattern, what entities have they mentioned before. This context went *into* the prompt, making extraction faster and more consistent.
...
This feature is what got us the most early interest. The workflow:

1. **Legal team creates an RFP** using a wizard I built - scope of work, budget ranges, timeline, evaluation criteria
...
I also built a **firm performance dashboard** that tracks metrics over time: billing compliance, response times, matter outcomes by firm. This gives legal teams leverage in negotiations - "your average response time is 3 days slower than other firms on similar matters."
...
**YC interview validation.** Got through the initial YC screening process - useful signal that the problem and approach resonated.
